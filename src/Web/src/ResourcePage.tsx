...
import { api } from './api'
import { resources, type ResourceKey, type Operation } from './resources'
...
export default function ResourcePage({ resourceKey }: { resourceKey: ResourceKey }) {
  ...
  return <>
    ...
    {operation && <Modal title={operation.op.label} close={() => setOperation(null)}>{queries.some(q => q.isError) && operation.op.fields?.some(f => f.source) ? <ErrorBox message="Não foi possível carregar os seletores. Atualize a página." /> : <OperationForm operation={operation.op} row={operation.row} lists={lists} done={() => { setOperation(null); setSuccess(true) }} />}</Modal>}
